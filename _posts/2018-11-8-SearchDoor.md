...
2. $$T_k$$  
$$T_k$$ is the total steps we move after $$k^{th}$$ iteration. For each iteration, we move left $$a_{k-1}$$ steps and then move right $$2a_{k-1}$$ steps. So that $$T_k=\sum^k_{i=1}3a_{k-1}=\sum^k_{i=1}\frac{3}{4}\cdot4^k=\frac{3}{4}\cdot4\cdot\frac{4^k-1}{4-1}=4^k-1$$.  
3. $$p_k$$  
...
Similarly, $$a_k=4^k$$.  
2. $$L_k$$ $$R_k$$   
Obviously, $$L_k=-a_{k-1}-1=-4^{k-1}-1$$, $$R_k=2a_{k-1}+1=2\cdot4^{k-1}+1$$.
3. $$p_k$$  
...
Therefore, we know that if the door is found in the $$(k+1)^{th}$$ iteration, it's either $$L_k>-n-1\geq L_{k+1}$$ or $$R_k<n+1\leq R_{k+1}$$. We can prove that, in the worse case, the door is to the left of the starting point and $$n=-L_k=4^{k-1}+1$$, which means $$4^k=4n-4$$. In this case, $$T(n)=T_k+p_k+n=\frac{3}{2}(4^k-1)-\frac{1}{2}+\frac{1}{2}\cdot4^k+n=2\cdot(4n-4)-2+n=9n-10$$. The constant multiple is $$9$$.
